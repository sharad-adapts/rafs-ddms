...
#   filename:  tensile_strength.1.0.0.json
#   timestamp: 2025-02-12T03:50:07+00:00
...
class TensileStrengthItem(RafsBaseModel):
    ...
    TensileStrengthMethodID: Optional[
        constr(
            regex=r"^[\w\-\.]+:reference-data\-\-TensileStrengthMethod:[\w\-\.\:\%]+:[0-9]*$"
        )
    ] = Field(
        None,
        description="The type of tensile strength test used, such as YongYu, ASTM (American Society for Testing and Materials), or Brazilian Disc.",
        title="Tensile Strength Method ID",
    )
...
class TestStep(RafsBaseModel):
    ...
    DataAcquisitionTime: Optional[float] = Field(
        None,
        description="Time that this test step was recorded.",
        title="Data Acquisition Time",
    )
    AxialForce: Optional[float] = Field(
        None,
        description="The force applied along the axis of a material specimen in a tensile test. This force is directed parallel to the length of the material and is responsible for stretching or elongating the specimen.",
        title="Axial Force",
    )
    AxialDisplacement: Optional[float] = Field(
        None,
        description="The amount of elongation or stretch experienced by a material specimen along its axis when subjected to an axial force during a tensile test.",
        title="Axial Displacement",
    )


class TensileStrengthModel100(RafsBaseModel):
    class Config:
        extra = Extra.forbid
    ...
    TVD: Optional[float] = Field(
        None,
        description="True Vertical Depth (TVD) is a measurement in the oil and gas industry that indicates the vertical distance from the surface to a specific point in a well. It is crucial for accurately locating geological formations, calculating pressures, and ensuring efficient and safe drilling operations.",
        title="True Vertical Depth",
    )
    TensileStrengthOrientationToBedding: Optional[float] = Field(
        None,
        description="Angle in degrees of the tensile strength relative to bedding",
        title="Tensile Strength Orientation To Bedding",
    )
    ...
    PeakLoad: Optional[float] = Field(
        None,
        description="The maximum amount of force or stress that a material can withstand during a tensile test before it begins to fail or fracture.",
        title="Peak Load",
    )
    TensileStrength: Optional[List[TensileStrengthItem]] = Field(
        None,
        description="The ability of a rock to withstand tension or pulling forces without breaking or failing. Essentially, it is the maximum stress that a rock can endure while being stretched or pulled apart before it fractures.",
        title="Tensile Strength",
    )
    ReservoirUnit: Optional[str] = Field(
        None,
        description="A free text string identifying the reservoir unit pertaining to this test.",
        title="Reservoir Unit",
    )
    TestSteps: Optional[List[TestStep]] = Field(
        None,
        description="Set of measurements captured in the tensile strength where each object represents a step with data measured",
        title="Test Steps",
    )
